...
with tf.device('/cpu:0'):
    ...
    def read_data(path,train = True):
        ...
        reference_images = []
        groundtruth_images = []
        for _temp_ in list_all:
            #_current_image_ = cv2.resize(cv2.imread(_temp_[0]).astype('float')/255, dsize=(320, 240))
            #_reference_image_ = cv2.resize(cv2.imread(_temp_[1]).astype('float')/255, dsize=(320, 240))
            #_groundtruth_image_ =  cv2.resize(cv2.imread(_temp_[2]), dsize=(320, 240))
